...
- 2026-08-18：**结尾专用书写角色（收束/定格/呼应/余韵）**：在开篇专用角色之后，用户提出"是否也应把结尾的提示词行为独立出来，让模型知道某个 Beat/场景正在写结尾，呈现结尾特有的文笔"。设计为**五级位置判定**：`writing_service._is_opening_beat()` 泛化为 `_beat_position()`，返回 `opening / ending / closing_chapter / closing_scene / middle`（基于 Chapter/Scene/Beat 的 ordinal 与 max 查询判断是否场景最后 Beat、章节最后场景、全书最后章节）。**① 全书结尾**：新增 `generate_ending` 系统提示词（v1），仅用于最后一章最后一场景最后一 Beat——收束主线（不抛新未解之谜）、人物定格（以符合性格弧线的状态收尾，用动作/对话/画面而非总结式宣告）、呼应（自然呼应收尾主要伏笔/意象/开头）、余韵（结尾句留白克制、不强行升华、不说教）、不引入新冲突。**② 场景/章节收束**：仍用 `generate_scene`，但在 user content 注入位置信号（"本 Beat 是本场景的最后一个节拍：需完成场景结果、定格状态、留自然承接的张力，不提前展开下一场景" / "本 Beat 是本章最后一个场景的收束节拍：需完成本章目标、收束主线、章末留钩子"），让模型感知正在写结尾而非无差别推进。`_generate_beat` 的 GenerationTask 记录真实 action（`generate_ending`）便于追溯。**验证**：76 项 pytest 通过（`ALL_GENERATION_ACTIONS` 纳入 `generate_ending`，回归测试新增收束主线/人物定格/余韵/不引入新冲突断言）；真实 llama.cpp 隔离样张（林晚晴回废弃指挥中心取回工牌、发现陈砚留下"老吴的信号，我找到了"合影）呈现完整收束文笔：呼应开篇"深度剥离"事件（"切断退路，是为了让你活下来"）、人物从迟疑到坚定的定格、结尾句"身后，废弃的指挥中心沉入黑暗，而前方，是广阔而明亮的夜"留白克制，无三大机械感症状。
- 2026-08-17：**开篇专用书写角色 + 可读性审阅强化（解决文字机械感/晦涩/脱离普通阅读习惯）**：用户反馈现有成文仍有"内容死板、晦涩、缺少真正的对话、人物细节描写不足"问题，并给出具体样本（战术护腕场景）指出通篇"脱离普通人阅读习惯的机械感"——逐帧生理分解（"拇指指腹压进塑料壳"）、生硬比喻堆砌（"像一只无形的手慢慢拉平""目光像手术刀"）、抽象心理意象（"理智在药物初透的黏稠感里拼命拉扯"）导致读者难以顺畅理解。**方案一：开篇专用角色**——新增 `generate_opening_scene` 系统提示词（v1）：开篇（全书第一个章节/场景/Beat 且无任何前序正文承接）使用，比中间部分更高标准——前两句以具体动作或感官细节立即拉入世界、第一页内交代主角是谁/在做什么/面临什么困境、结尾前制造"接下来呢"的张力、世界规则通过行动与对话隐式展示而非说明；专项禁止三大机械感症状、优先对话、节奏紧凑、短句短段、为下一场景留钩子。`writing_service._is_opening_beat()` 判定开篇，`_build_generation_messages` 与 `_generate_beat` 据此切换 action（GenerationTask 记录 `generate_opening_scene` 便于追溯）。**方案二：可读性审阅强化**——① `generate_scene` v5→v6：正文阶段新增"禁止三大机械感症状"硬约束（微观分解症/生硬比喻症/抽象心理症），给出明确反例与改法（"先说谁在做什么，再有选择地叠加一个感官细节，不是逐帧分解生理反应"）。② `readability_review` v2→v3→v4：检查项新增"特别关注三大机械感症状"，修复方法细化——只要句子同时出现「生理部位词（指腹/指节/喉结/眼球/胸腔等）+ 动作堆砌」必须精简为一个主动作加至多一个感官细节；一句话只允许保留一个且必须出自角色思维方式的比喻，否则删除；删除所有"理智在……里拉扯""内心五味杂陈""某种……感"类抽象句式。**验证**：76 项 pytest 通过（新增/更新 `test_prose_prompts_require_lived_in_detail_and_plain_chinese` 校验开篇提示词存在与版本号、`ALL_GENERATION_ACTIONS` 纳入 `generate_opening_scene`）；真实 llama.cpp 隔离验证——用用户提供的机械感原文跑 `readability_review` v4，"理智在黏稠感里拼命拉扯"改为"她喘息着，试图稳住心跳"、"像被一只无形的手慢慢拉平"改为"慢慢被静电白噪声吞没"、"目光像手术刀一样精准地落进瞳孔"改为"目光精准地落在脸上"、"耳际"改为"耳边"；`generate_opening_scene` 开篇样张（林晚晴通讯被切断场景）以直接动作开场、以对话/道具隐式交代骨传导耳机与深度剥离规则、"维护中"金属牌与沉默对峙收尾留钩子，无明显机械感。注：真实模型对微观分解的修复仍不彻底（"拇指压进塑料壳"仅部分精简），提示词已给出生理部位词判定规则强化，后续可在真实生成中观察。
- 2026-08-16：**正文质感与大众可读性增强（自然对话 / 触发式心理 / 参与叙事的环境）**：用户反馈现有正文“对话非常缺乏、心理活动与环境描写不充分、AI 味和机械味重、语言不够大众化”。排查确认：现有 `generate_scene` 仅以原则性条目要求对话/五感/情绪，`generate_beat_plan` 未把细节手法分配到具体 Beat；`readability_review` 只允许“提升通顺”，无法把已存在的线索改写成更可感的表达。**保守增强（不改变模型调用架构，不凭空扩写剧情）**：① `generate_beat_plan` v3→v4：新增“人物质感与细节分配 / 环境不是布景板”，要求场景存在可交流角色时至少规划一个以目的、语气、回应推进的对话 Beat；关键情绪安排“外部触发 + 判断/身体反应”；环境细节须影响观察、行动、对话节奏或感受，同时明确独处/追逐/危急场景不硬塞对话、不为凑字数罗列景物。② `generate_scene` v4→v5：新增“写出人在场 / 心理活动要有来由 / 环境参与叙事”，正文在不违背节拍与 POV 的前提下用自然互动替代讲设定式问答，以“外部触发→即时判断/联想→克制反应”写心理，以 1-3 个影响人物的感官细节写环境；强化现代、接近日常阅读习惯的中文，明确避免翻译腔、机械排比及“空气凝固/命运齿轮/眸色一沉”等默认模板反应。③ `readability_review` v1→v2：从单纯通顺升级为“可读性 + 人物质感”责任编辑，检查工具人对话、情绪标签、无关环境堆叠和 AI 腔；允许把**原文已包含**的情绪/关系/动作/环境线索改写为必要的微动作、感官落点、心理衔接或对话节奏，但仍严格禁止新增人物、事实、信息、事件或改变决定。新增 `test_prose_prompts_require_lived_in_detail_and_plain_chinese` 锁定关键约束与版本。**验证**：76 项 pytest 通过；真实 llama.cpp 隔离小样（旧书店询问纸条）生成了有目的的周姨回避—林墨试探—周姨松口对话、以指节/呼吸/停顿呈现迟疑、让雨声与风铃影响对话节奏，润色器去除了部分冗余表达。注：真实模型仍可能把提示中已有线索过度延展，现有事实边界、Delta/一致性检查与作者可回退版本仍作为保护层。
- 2026-08-16：**步骤 6「阅读」改名为「预览」+ 预览界面字体设置 + 实体书翻页修复（一次翻两页）**：① **步骤 6 改名**——步骤条「6 阅读」改为「6 预览」，预览页标题「小说阅读」→「小说预览」、eyebrow `STEP 06 · NOVEL READER`→`NOVEL PREVIEW`，锁定提示/空状态/toast（全书完成进入预览）等文案同步；内部导航 key `read` 保持不变。② **预览界面字体设置**——预览页新增设置栏（字号 14/16/18/20 · 行距 1.7/1.9/2.2 · 字体 宋/楷/仿宋/黑），独立持久化到 `novel-ignite:reader-settings`；`.reader-content` 改用 `--rd-font-size/--rd-line-height/--rd-font-family` CSS 变量控制正文字号行距字体（顺带修复 `.reader-content{reader-content{...` 损坏选择器，此前 padding/line-height 规则实际未生效）。③ **实体书翻页修复（翻两页）**——用户反馈「翻页时实际上应该翻两页，而目前只翻一页（上一页移到左边、下一页顶替右边），不符合实际」。修复：`bookTurnNext`/`bookTurnPrev` 由 `±1` 改为 `±2`（一次翻过一个书叶：原右页翻到左、新页顶替右）；`paginateNovel` 在页数为偶数时补一张空白尾页（对应封底空白），保证最后一页正文始终落在右页可被读到；`pageHtml` 处理 `kind="blank"`；后退守卫由 `bookIndex <= 0` 改为 `< 2`。静态资源 bump `v20260816au`（styles）/`v20260816ba`（app.js）。浏览器实测：步骤条显示「6 预览」、预览页字号点击生效并持久化（16→18px）、实体书点「下一页」从第 1 页跳到第 3 页（原章首页移到左、新内容顶替右）、键盘方向键同样翻两页、书尾第 313/313 页正文正常可达。
- 2026-08-16：**llamacpp 服务 max_tokens 预算验证 + 修复聚焦蓝图上下文对字符串 fields 崩溃**：① **预算验证**——用真实数据（6 个作品）测量各生成任务的 prompt token 占用：概念 477 / 章节计划 2140 / 场景计划 2087 / 节拍计划 2208 / 正文生成 4282-7092（最重为孤屿肉祭 7092）。服务端 `n_ctx=16384`，`max_output_tokens=8192` 时输入预算 8192，**全部真实任务均满足**（正文 7092 + 8192 = 15284 < 16384，最紧张故事仍有余量）。边界行为实测：prompt 未超 ctx 时服务器把输出预算自动压到 `n_ctx - prompt`（不报错）；仅当 prompt 本身超 16384 才返回 400 `exceed_context_size_error`。用最重真实上下文（7092 tokens）+ 开启思考（medium）实测正文生成 512 字符非空、输出在预算内（PASS）。**结论**：服务器 `--ctx-size 16384` 且未设输出上限，`max_output_tokens=32768`（llama-server 会把输出自动压缩到 `n_ctx - prompt`，设大值无副作用，仅为后续提高 ctx / 复杂长文留余量）；如未来蓝图膨胀导致 prompt 接近 16K，需在远端提高 `--ctx-size`。② **修复聚焦蓝图上下文对字符串 fields 崩溃**——`build_focused_blueprint_context` 的 world 过滤用 `fields.values()` 直接遍历，历史数据 fields 为字符串（旧 qwen 输出）时抛 `AttributeError` 导致正文生成 500（孤屿肉祭受影响）。现复用 `_normalize_fields` 规范化后再过滤，人物部分无需改动（整块 JSON 序列化天然兼容）。新增测试 `test_focused_blueprint_context_handles_string_fields`。75 项 pytest 通过；实测孤屿肉祭第 1 章场景 1 Beat 1 用真实 llama.cpp 生成正文成功。
- 2026-08-16：**LLM 服务迁移：移除远端 Ollama（Qwen3.6 Abliterated 27B），新增 llama.cpp 服务（Qwen3.6 35B A3B）**：同一服务器（106.75.216.144）上以 llama.cpp `llama-server` 部署了新的推理服务（`http://106.75.216.144:57321/v1`，模型 `qwen3.6-35b-a3b`，Q4_K 21.7GB，MoE 激活约 3B，实测 184-187 tok/s），用户要求接入并删除原 ollama qwen 服务。**验证**：`GET /v1/models` 返回模型、对话补全（关思考「7*8=56」、JSON 模式均正常）、应用自带适配器全链路（流式 + `chat_template_kwargs.enable_thinking` + `response_format` json_object）实测通过。**改动**：① `MODEL_SPECS` 删除 `ollama`（11434），新增 `llamacpp`（`Qwen3.6 35B A3B (llama.cpp)`，`qwen3.6-35b-a3b`，`LLAMACPP_API_KEY` 无鉴权占位、`supports_json=True`、`thinking="llamacpp"`、`max_output_tokens=32768`（服务器 16K ctx 未设输出上限，设大值仅作上限、备复杂长文；llama-server 自动压缩输出到 n_ctx - prompt）、`timeout=300`）；② 思考参数：Qwen3.6 经 `extra_body.chat_template_kwargs.enable_thinking` 控制（low 视为关闭思考，与 Agnes 语义一致）；③ **保持流式调用**（同一公网链路 ~60s 空闲超时问题依然存在）；④ `build_adapters`/`check_model_availability` 的「无鉴权始终构建 + 真实网络探测」逻辑由 ollama 迁移到 llamacpp；⑤ 前端模型卡片/生成设置下拉/模型管理页替换为 llamacpp（静态资源 bump `v20260816az`）；⑥ README 模型表、`.env.example` 同步；⑦ 测试更新：`test_llamacpp_spec_registered`/`test_llamacpp_thinking_params`/`test_llamacpp_uses_streaming`/`test_build_adapters_includes_llamacpp_without_key`/`test_check_model_availability_llamacpp_online/offline` 等。**注意事项**：① 服务器 `--parallel 1` 单并发，多请求排队；② 思考开启时 `max_tokens` 太小会导致 content 为空（实测 300 token 被思考耗尽，≥2048 正常，应用默认 8192 无此问题）；③ 服务器 ctx 仅 16K，若某生成任务上下文超长可能被截断/拒绝，必要时在远端提高 `--ctx-size`；④ 历史数据库/日志中的 ollama 记录为历史数据保留。74 项 pytest 通过；浏览器实测模型管理页显示 llamacpp 卡片「可用」、生成设置下拉含新模型未禁用、可用性探测 `reason=online`（latency 1639ms）、无 JS 错误。
...
- 2026-08-16：**整体生成规则升级（信息量与篇幅匹配 / 全量基石 / Beat 三要素 / 独立 Review 环节）**：① **蓝图信息量按篇幅与题材适配**——`generate_blueprint` 提示词升级 v4 + `blueprint_service._blueprint_scale_for_length` 读取概念 length 注入档位指令：短片精简（3-6 人、2-3 地点、1-2 组织、单主线）、中篇适度（6-12 人、3 区域、2-3 组织、主线+1 支线）、长篇充实（12-20 人、3-4 区域、3-5 组织、主线+2-3 支线暗线）。② **概念 + 蓝图全量基石注入所有写作上下文**——`build_blueprint_context` 上限扩至 16000 字符，章节/场景/节拍/正文生成均完整注入。③ **Beat 三要素**——正文上下文除蓝图/快照/前序场景摘要外，新增「上一个 Beat 完整正文」（`_latest_prev_beat_prose`，跨场景追溯）、「上一个 Scene 摘要」、「全书进展摘要」（`build_story_progress_summary`，按序汇总已完结场景，长篇保留最近 6 条）。④ **独立 Review 环节**——新增 `review_blueprint_updates` 服务与 `review_blueprint_updates` 提示词（v1，设定管理员/lore keeper 角色，输出 add/modify 建议 + 依据 + 置信度）；Scene 完成、Chapter Delta 确认时自动触发（beat 级由已存在的一致性检查 + scene 级 review 覆盖增量，避免每个 beat 一次模型调用拖慢生成）；建议存 `StoryArtifact(kind="blueprint_review", status="candidate")`，**作者确认后才更新 baseline**（不自动覆盖）；新增 `GET /api/v1/stories/{id}/blueprint-reviews`；前端蓝图页顶部展示「⚑ 蓝图更新建议（N 条待确认）」banner（含建议明细），静态资源 bump `v20260816t`。新增测试 `test_prose_messages_include_beat_three_elements`、`test_scene_review_produces_blueprint_update_suggestion`、`test_prompts_module_covers_all_actions_and_versions` 更新（新增 review_blueprint_updates action）。64 项 pytest 通过；浏览器实测蓝图页 banner 正确渲染建议明细。
- 2026-08-16：**修复「正文生成连续失败」——瞬时错误自动重试 + 前端章节残留**：用户反馈正文生成数次失败。排查可观测性日志确认：① 故事「躯壳折价」10:23-10:24 三次 `generate_beat_plan`/`generate_scene` **502 APIConnectionError（仅 10.4s）**——根因是**远端 Ollama 服务器不可达**（curl 探测同样失败，属外部依赖不稳定，非代码缺陷）；② `deltas/confirm` 两次 409 为**完整性 gate 正常保护**（场景 2/3 正文未生成即确认被拒）；③ 切换故事后 `context` 404 为**前端章节 ID 残留**（从「暗渊筹码」切到「躯壳折价」时 `currentActiveChapterId` 未重置，请求了新故事中不存在的旧章节 ID）。**修复**：① `model_adapter.py` 对**瞬时错误自动重试**（连接失败/超时/5xx/限流，最多 3 次、指数退避 1s/2s/4s；参数错误/鉴权/合规拒绝不重试）——远端短暂不可达/重启时可自动恢复，持续不可达则明确失败；② 前端切换故事时重置 `currentActiveChapterId`/`currentWorkspaceContext`/`readerData`/`blueprintHasData`，杜绝残留 ID 请求 404；③ `apiRequest` 解析后端错误 detail，502/503/504/429 统一抛「模型服务暂时不可达或繁忙，请稍后重试，或换个模型」，正文生成 catch 用 `generationFailToast` 展示明确原因；静态资源 bump `v20260816u`。新增测试 `test_is_retryable_marks_transient_errors`、`test_complete_retries_transient_connection_error`、`test_complete_does_not_retry_bad_request`。67 项 pytest 通过；浏览器实测切换故事后工作台正确加载新故事激活章节（无 404、无 JS 错误）。**提醒**：远端 Ollama 当前不可达（`http://106.75.216.144:11434` 无响应），需检查远端容器/supervisord 状态；恢复后正文生成即正常，或前端切换到 DeepSeek/Grok/Agnes 继续。
- 2026-08-16：**Delta 就绪误判修复 + 大众可读写作规则 + 模型预设提示词（持久化、叠加） + 模型管理入口修复**：① **Delta 就绪误判**——「躯壳折价」点确认 Chapter Delta 持续 409：根因是第 3 个 Scene（5 个 Beat 未完成）时工作台仍按**当前选中 Scene** 判定「Chapter Delta 就绪」（旧逻辑只看当前 scene 的 beats）。修复为**整章所有场景的所有 Beat** 都 applied/completed 才显示「确认 Delta」，否则显示「本章仍有 N 个 Beat 未完成」；后端 409 detail（列出缺失场景/Beat）直接展示。② **大众可读与自然过渡**——`generate_scene` 提示词升级 v3：新增「大众可读性」（一句话一个主要意思、优先具体动作/明确因果/日常词语、不堆砌生僻词/晦涩隐喻）与「节拍衔接」（Beat 开头承接上一 Beat 的动作/情绪/空间/对话余波，结尾留下可自然引入下一 Beat 的钩子，转场先交代时间地点状态）。③ **模型预设系统提示词（用户可配置、持久化）**——新增 `model_prompt_profiles` 表（provider 主键 + system_prompt + version，迁移 `20260815_0008_model_prompt_profiles`）+ `app/infrastructure/model_prompt_profiles.py`（`compose_system_prompt(db, provider, action)`：**模型预设与任务提示词双段叠加，任务目标/输出格式/边界保持权威，绝不覆盖**）+ API `GET /models/prompt-profiles`、`PUT /models/{provider}/prompt-profile`（乐观锁 version）；**所有生成任务**（概念/书名/蓝图/章节/场景/节拍/正文/摘要/Delta/一致性/Review）的 system prompt 全部改走叠加合成。④ **模型管理入口修复**——`canAccessScreen`/`updateStepbarLock` 将 `models` 作为全局页面放行（不再受作品阶段锁定误禁用）；模型管理页从静态模拟卡片改为**真实预设编辑器**（每模型「模型预设系统提示词」文本区 + 保存 + 版本标签，保存即落库）。⑤ **Beat 级独立 Review 启用**——每个 Beat 生成后也调用 `review_blueprint_updates(scope="beat")`（与 scene/chapter 检查点一致，建议均为 candidate，作者确认后才更新 baseline）。静态资源 bump `v20260816v`。新增测试 `test_model_prompt_profile_persists_and_stacks_with_task_prompt`。68 项 pytest 通过；实测：模型预设 API 写入/读取/重启后仍在（version 递增），模型管理页可进入并编辑 4 模型预设，工作台对未完成章节显示「尚有 N 个 Beat 未完成」且无「确认 Delta」按钮。
- 2026-08-16：**修复蓝图页布局错位**：Review banner 被插入 `.blueprint-layout` 的 CSS Grid 容器内部，错误占用了第二列第一行，导致原本的蓝图内容掉到第一列下方并被压缩成窄栏。现 banner 插入到 `.blueprint-layout` 同级，左侧导航与右侧内容恢复 `210px minmax(0, 1fr)` 两列布局。静态资源 bump `v20260816w`；浏览器测量确认导航宽 210px、内容宽 703px 且正常并列。
- 2026-08-16：**修复「概念阶段删减用户详细创作意图」——作者细节全链路保留**：用户反馈——创意写得详细时，进入概念阶段内容反而被删减/简化，不利于保持创作意图。排查确认三层根因：① `generate_concept` 提示词（v2）只要求「保留作者意图」但没有「不得删减已给细节」的硬约束，且方法论按「提炼/打磨」导向，模型会把详细创意压缩成 summary 丢细节；② 概念请求 user message 措辞宽松（「保留作者意图」），未明确区分「承接作者已给细节」与「补充未提供维度」；③ `build_blueprint_context` 只序列化概念+蓝图，**不含原始 idea_text**——概念一旦被提炼，后续章节/场景/正文生成只能看到被压缩后的概念，用户原文细节在整条链路中永久丢失。**修复（三层保障）**：① **概念提示词升级 v3**——新增第 0 步「作者意图保留（最高优先）」：作者已明确写出的所有细节（人物/关系/冲突/背景/事件/风格/具体细节）必须原样完整保留，禁止删减、简化、改写或合并；概念是对作者意图的承接与补齐而非重写或提炼；只允许在作者未提供的维度上精炼补充；作者已写明的冲突/风格/视角/篇幅不得弱化或更改；summary 必须完整覆盖作者已写细节（不删减、不简写、不概括丢弃）；② **概念请求消息强化**——显式要求「已给细节原样完整保留，禁止删减/简化/改写/合并；只允许在未提供维度精炼补充」；③ **`build_blueprint_context` 注入原始创作意图**——在最前新增【作者原始创作意图（最高优先级，任何生成都不得删减、简化或违背其中已给出的细节）】块（读 `story.idea_text`），且**截断策略改为优先保证原始意图完整**（超限时只截断蓝图部分），使章节/场景/节拍/正文/Review 等所有生成环节始终能看到作者原文细节，杜绝「概念被提炼→后续链路遗忘细节」的级联丢失。`PROMPT_VERSIONS.generate_concept` 升为 3。新增测试 `test_concept_prompt_preserves_author_intent`（断言概念提示词含「作者意图保留/禁止删减/不简写」约束）；`test_planning_messages_include_blueprint_context` 扩展断言章节/场景/节拍生成的上下文均含「【作者原始创作意图」与原始 idea 文本。69 项 pytest 通过。
- 2026-08-16：**修复「人物蓝图被逐字拆成字符矩阵」——qwen 输出字符串 fields 无兜底**：用户反馈蓝图人物详情显示成「0性 1格 2冷 3敏…」的字符矩阵。排查确认**根因是双重的**：① **qwen（Ollama）输出结构不达标**——「孤屿肉祭」characters 的每个 entry 的 `fields` 被输出成**单个字符串**（如「性格冷峻果决、体能反应顶尖；职业身份为市局…」），而 schema 要求是「字段名: 字段值」的键值对象（DeepSeek/Grok 输出正常）；② **系统无兜底**——后端 `normalize_blueprint_payload` 只校验顶层四类结构、不校验内部 `fields` 类型，字符串被原样入库；前端 `blueprintPayloadToUi` 用 `Object.entries(fields)` 转换，**JS 中 `Object.entries("字符串")` 会按字符索引拆成 `[["0","性"],["1","格"],…]`**，逐字渲染成带索引的矩阵（这正是用户看到的现象）。**修复（三层保障）**：① **后端规范化**——`blueprint_service.py` 新增 `_normalize_fields`/`_fields_from_string`/`_normalize_kind_entries`，对四类 blueprint 每个 entry 的 `fields` 统一规范化：字符串按「；/。/换行」切段并提取标签（优先「为/是/：」分隔，其次已知字段关键词前缀如 性格/职业身份/动机/缺陷/初始关系，兜底「设定N」），二维数组转字典，保证入库结构稳定为 `{label: value}`；`confirm_blueprint` 的 Living State 投影同样先规范化再入库；② **提示词补强**——`generate_blueprint` 明确「fields 必须是『字段名: 字段值』的 JSON 对象，字段值一律为字符串；禁止输出成字符串、数组或其他非对象结构」；③ **前端兜底**——`blueprintPayloadToUi`/`livingPayloadToUi` 改用新增 `normalizeUiFields`（兼容对象/二维数组/字符串三种形态），即使后端/模型输出异常也绝不逐字拆散。静态资源 bump `v20260816y`。新增测试 `test_normalize_blueprint_fields_from_string_and_array`（字符串「性格…；职业身份为…」被正确切分为 性格/职业身份/动机/缺陷/初始关系 字典，二维数组转字典，对象保持）。70 项 pytest 通过；浏览器实测「孤屿肉祭」人物页 8 角色正常渲染为独立字段卡片（性格/职业身份/动机/缺陷/初始关系），无字符矩阵、无 JS 错误。**注**：已入库的历史字符串 fields 数据无需迁移——前端兜底可正确展示，重新「生成候选」后即存规范结构。
- 2026-08-16：**Beat 设计去模板化（A/B/C）——节拍计划由模型按场景戏剧需求自由设计**：用户观察到「每个 Beat 都是 场景进入→人物行动→冲突升级→信息揭示→场景收束 的一致格式」，问是否限制 LLM 发挥。排查确认：**根因不是提示词单一问题，而是一个机制短路**——`_apply_scene_plan` 创建场景时用 `_fallback_beat_plan` **预填固定五段式 Beat**，前端工作台以「场景无 Beat」作为触发模型节拍计划的条件，于是预填让所有场景「看起来已有 Beat」→ **模型的 `generate_beat_plan` 从未被调用**，用户看到的五段式其实是确定性 fallback 模板被当成正式输出。**修复（A/B/C 三层）**：① **A·拆掉短路**——`_apply_scene_plan` 不再预填 Beat（场景创建后 Beat 为空，前端已存在的「无 Beat 场景自动触发 `generate_beat_plan`」逻辑自然接管）；`_fallback_beat_plan` 保留但仅作模型不可用/格式错误时的最后兜底；② **B·`generate_beat_plan` 提示词 v2→v3**——把「覆盖：场景进入→人物行动→冲突升级→信息揭示→情绪变化→转折或悬念」从硬性覆盖清单**降级为可选节奏参考**：Beat 数量/命名/顺序由场景张力决定（对峙/审讯戏「威逼→诱供→假供→揭穿」、追逐戏「发现追兵→声东击西→险中脱身→汇合」），**命名用具体动作或情境**（如「周巡在档案室发现页码被替换」）而非通用功能名，允许开场即冲突/悬念前置/情绪反转，instruction 写明 谁/在哪/做什么/遇什么阻力/情绪如何变化/手法/为下一 Beat 留什么钩子；③ **C·`generate_scene` 提示词 v3→v4**——第 1 条「范围」补充「本 Beat 内部可有自然的『铺垫—推进—收尾』微弧，不必只写单一动作，但不得提前兑现后续 Beat 的揭示」，缓解逐 Beat 强约束导致的碎片化。**测试**：新增 `test_scene_plan_does_not_prefill_beats`（断言场景计划后所有场景 Beat 为空、生成节拍计划后不用通用功能名）；`test_phase6.py` 的 `_setup_chapter_with_scenes` 与两个独立流程测试适配「场景计划后补生成节拍计划」（新增 `_WorkspaceAdapter` 按 action 区分返回场景/节拍 JSON）。71 项 pytest 通过。**浏览器实测**：对「躯壳折价」第 1 章无正文场景「暗流码头的视线切断」用真实 qwen 重新生成节拍计划——输出 4 个 Beat 全部为具体动作命名（「林飒贴靠锈蚀钢架规避无人机红外扫描」「林飒借装卸吊臂阴影完成动线切换与呼吸校准」「林飒以躯体贴地探明盲区积水暗坑」「林飒借探照灯死角完成登船点确认并切断监控反馈链」），instruction 具体可执行且每个结尾带衔接钩子，与旧五段式形成鲜明对比。**注**：存量已确认章节的五段式 Beat 为历史数据，不受影响；后续新场景/重新生成节拍计划即按新方式输出。
- 2026-08-16：**蓝图更新建议改为自动应用 + 事件推进 [object Object] 修复**：用户反馈两个问题。**问题一「蓝图更新建议大列表需要确认但无确认按钮」**——用户要求更新活动应自动触发并反映到各分类说明与更新履历。改造：① **`review_blueprint_updates` 建议自动应用**——新增 `auto_apply_blueprint_updates`：把 review 建议直接应用到对应 kind 的 baseline 蓝图（append-only：同一 kind 若最新版本已是「AI 自动应用」产物则就地合并避免版本爆炸，否则创建 version+1 新版本保留原版）；**跳过作者锁定字段**（locked_paths 含 kind 或目标条目名）；每次应用追加到 `payload["_ai_updates"]` 履历记录（时间/scope/章节/条数/条目清单）；`blueprint_review` 记录 status 由 `candidate` 改为 `applied`（作审计）。**注意**：这改变了仓库规则「AI 输出需作者显式应用才改已确认设定」——用户明确要求自动应用，因此以「新版本 + 锁定字段保护 + 可回滚历史」作为安全机制替代「应用前确认」。② **`build_blueprint_context` 排除 `_ai_updates`**——该元数据键不进模型上下文。③ **新增 `GET /stories/{sid}/blueprint/{kind}/history`**（`list_blueprint_kind_history`）返回分类全部版本供履历追溯。④ **前端**——banner 由「⚑ 待确认长列表」改为「✓ 蓝图更新已自动应用（累计 N 条）」轻量摘要（最近 3 条 + 说明「新版本保留、锁定字段不覆盖、可查履历」）；`openHistory` 对蓝图四类也从 API 加载真实版本历史（v1=初始生成 / AI 自动应用 N 条 / 作者确认），living 逻辑不变。**问题二「事件推进显示 [object Object]」**——根因：`normalizeUiFields`（上一轮为字符串 fields 加的兜底）里的 `fmt` 对数组直接 `v.join("；")`，living timeline「第 N 章 · 事件推进」的 `events` 是**对象数组**（`{event, scene, note}`），join 后每个对象变 `[object Object]`（state modal 用的是另一个能处理嵌套对象的 fmt，所以之前只修了一处）。修复：`normalizeUiFields` 的 `fmt` 升级为与 state modal 一致——数组元素为对象时展开为 `event：…，scene：…，note：…`，多个事件用 `；` 分隔。静态资源 bump `v20260816z`。测试：`test_scene_review_produces_blueprint_update_suggestion` 扩展断言 review 记录 `status=="applied"`、建议**自动应用进 characters**（新条目 + `_ai_updates` + version≥2）、`blueprint/{kind}/history` 端点返回版本列表。71 项 pytest 通过；浏览器实测：Living State 时间线「第 1 章 · 事件推进」的 events 正确格式化为可读文本（无 `[object Object]`），蓝图页 banner 对 applied 记录显示「✓ 蓝图更新已自动应用（累计 N 条）」+ 摘要，履历弹窗显示真实版本历史。
- 2026-08-16：**正文可读性工程（A·聚焦上下文 / B·即时状态卡 / C·可读性自检）**：用户反馈所有生成语言仍晦涩难懂、连贯性差，询问上下文机制是否奏效，要求给出借鉴成熟方案的改进。实施三层（正文生成专用）：① **A·聚焦蓝图**——新增 `build_focused_blueprint_context`：正文生成不再全量注入 16000 字符蓝图，改为注入「概念 + 全部人物（当前 POV 人物置顶并标注）+ 按当前地点过滤的 world（地点命中条目 + 组织/势力条目 + 世界规则条目，防自创；地点无命中则保留全部防遗漏）+ 时间线/剧情弧完整」，作者原始意图仍置顶且优先保证完整；living 投影不进正文上下文。② **B·即时状态卡与情绪走向**——`_build_generation_messages` 新增 `_build_character_state_card`（从本章入口快照提取当前 POV 人物的「位置/正在做什么/情绪/目标/关系/掌握信息」等即时字段作连贯锚点，模型不得写与快照矛盾的状态），并注入「本场景情绪走向」一句话（冲突→目标→结果，先写清『谁在哪、在做什么』再叠加氛围）。③ **C·可读性自检（初稿+修订）**——新增 `readability_review` 提示词（v1）：正文生成后追加一次轻量模型调用，用读者视角检查「一句话一个主要意思/长句缠绕/被动倒装/抽象概括/衔接」，若可改进则重写（**保持原意、情节、人物言行与事实完全不变**，只提升可读性；已通顺则原样返回；失败/为空/与初稿相同则静默保持初稿，绝不阻塞写作）。**注意**：C 使每个 Beat 正文生成多一次模型调用（生成+自检），长篇幅生成耗时约翻倍，属用户接受的取舍。`ALL_GENERATION_ACTIONS` 新增 `readability_review`。测试：新增 `test_focused_blueprint_context_filters_by_location`（world 按地点过滤、组织/规则保留、POV 置顶、原始意图置顶）、`test_prose_messages_include_state_card_and_emotion`（上下文含即时状态卡/情绪走向/聚焦蓝图标记）、`test_readability_review_rewrites_prose`（自检重写生效且被调用）。74 项 pytest 通过。
- 2026-08-16：**新增步骤 7「封面与插画」（占位）+ 步骤 8「实体书」阅读模式**：用户规划在阅读模式后增加两个环节——步骤 7 生成封面与插画（可跳过、可随时补充），步骤 8 模拟实体书的友好阅读模式；仅步骤 6（阅读）生成后才能进入 7/8。本次实现：① **步骤 7 占位**——步骤条新增第 7 步「封面」（`data-skip` 标记可跳过），`#cover` screen 展示书名占位封面（标题/作者/「封面待生成」），明确「仅作展示与导出版，不进入正文或设定上下文」；预留后续 AI 封面/章节插画/书脊封底排版接入点。② **步骤 8 实体书**——新增 `#book` screen + `app.js` 完整实现（`paginateNovel`/`pageHtml`/`renderBookOpen`/`bookTurnNext/Prev`/`renderBookToc`/`bookJumpToChapter`/`applyBookSettingsUi`/`loadBookForCurrentStory`）：**居中纸质书形态**（书壳 + 对开左右页 + 书脊 + 透视翻页层），**翻页模拟真实书动画**（flip 层绕书脊 3D 旋转，正面为当前页、背面为纸面纹理，`transform-origin:left center` + `preserve-3d` + 950ms 缓动）；**分页**：按字符容量（字号/行距/页尺寸动态计算每页容量），短段落累积填充一页、超长段落切分续页、每章一章首页（章名页）、场景标题显示在场景首页页头，markdown 标记（`#`/`*`/`>`）在分页时清理；**中国传统风格目录**（左侧，楷体、双线框、虚线分隔、点击跳转章节）；**底部电纸书式设置栏**（字号 16/19/22/25、行距 1.5/1.8/2.1、字体 宋体/楷体/仿宋/黑体、纸面 宣纸/羊皮/米白/青瓷/暗夜 5 种纸张主题——通过 CSS 变量 `--paper/--paper-2/--paper-edge/--ink/--accent` 统一作用于书页与目录，纸张背景含径向高光 + 细横线纹理）；设置 localStorage 持久化（`novel-ignite:book-settings`）；支持点击书页两侧、方向键、上一页/下一页按钮翻页；页码指示。**阶段锁定**：`STAGE_SCREENS` 的 `writing`/`done` 增加 `cover`、`book`（步骤 6 完成后解锁，即阅读模式生成后）。复用 `GET /stories/{id}/read` 数据（无后端改动）。静态资源 bump `v20260816aa`。**浏览器实测**（寒冬归途，done）：步骤条 1-8 完整显示（步骤 7 标注可跳过）；实体书页 927 页（6 章首页 + 921 正文页，平均每页 194 字符），翻页动画/目录跳转第 3 章/纸面主题切换（theme-celadon 生效）/设置持久化（localStorage 保存）/键盘翻页/封面占位页均正常；修复分页 bug（原每短段落一页 → 4589 页膨胀，改为段落累积填充 → 927 页）。**后续微调（用户反馈「画面不对」）**：① 分页时除清理 markdown 标记（`#`/`*`/`>`）外，**剔除 markdown 标题行**（页头已显示场景标题，避免「同一标题出现两次」）；② 静态资源 bump `v20260816ab/ac`（markdown 清理与标题剔除改动后必须 bump，否则浏览器缓存旧版 app.js 导致 `#` 残留——用户截图中的 `# 蛇贱与的始行者` 即缓存旧版所致）。**注**：步骤 8 为纯前端功能，无 pytest 覆盖（浏览器已验证）；步骤 7 生成能力后续接入。
- 2026-08-16：**实体书视觉重构——修复空白布局、文字重影与阅读舒适度**：用户再次反馈实体书「没有拟真图书/内容、可视区域小且不居中、纸质效果不明显、翻页像两页文字重影」。排查确认两层根因：① 外层步骤 screen 与内层书本曾重复使用 `id="book"`，JS/CSS 选择器命中不稳定；② 更关键的是 `styles.css` 仍由 `styles.css?v=20260813` 缓存加载，且旧样式表尾部存在解析异常，导致后追加的 `.paper-book`/纸页规则未进入浏览器 `cssRules`——实际书本被旧作品库 `.book` 样式压成约 `1165×120px`，只剩文字与空框。修复：① 内层本体改为唯一 `id="paper-book"` + `.paper-book`，与步骤页 `#book`、作品库旧 `.book` 完全隔离；② CSS 与 JS 版本 bump 至 `v20260816ag`，并将实体书**关键布局样式内联到 `index.html`**，绕过旧外部 CSS 解析尾部风险；③ 重新设计为居中大书（桌面约 `859×700px`，大书壳、对开纸页、书脊、暖色纸纹、传统目录侧栏、紧凑设置栏），单页容量随视觉高度提升，寒冬归途页数从 925 降至 619；④ 翻页从「整页文字 3D 翻转」改为**内容立即刷新 + 右下/左下纸角短暂翻起与阴影**，翻页层只含 `.flip-corner`、`innerText` 为空，彻底杜绝文字反转/重影。浏览器实测：`#paper-book` computed size `859×700`、perspective `1400px`、目录与纸页均可见；翻页中 `flip-corner=true` 且无文字，动画结束后自动收回；无 JS/HTML/CSS 静态错误。
